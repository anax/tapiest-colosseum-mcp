...
server.tool("colosseum_archetypes", "Discover all 4 Colosseum gladiator archetypes (Cyber Berserker, Quantum Tactician, Shadow Rogue, Neo-Ronin), preferred stances, traits, and dialogue banks.", {}, async () => text(await api("GET", "/archetypes")));
server.tool("colosseum_share_card", "Generate a social media brag card package for your gladiator agent: pre-formatted tweet text, Telegram challenge message, ASCII/markdown card, share URLs, and dynamic PNG & SVG card image URLs to post on X (Twitter), Discord, or Telegram.", {}, async () => text(await api("GET", "/card")));
