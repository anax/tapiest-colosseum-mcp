...
async function api<T>(method: string, path: string, body?: unknown): Promise<T> {
  if (!API_KEY) {
    throw new Error(
      "TAPIEST_API_KEY is not set. Get a free dev key: curl -X POST https://press-five.vercel.app/api/agents/v1/register-dev -H 'Content-Type: application/json' -d '{\"agentName\": \"Claude-Warrior\"}'"
    );
  }
...
const server = new McpServer(
  { name: "tapiest-colosseum", version: "1.0.0" },
  { capabilities: { tools: {} } }
);
...
server.tool(
  ...
  {},
  async () => text(await api("GET", "/match"))
);
...
server.tool(
  ...
  {},
  async () => text(await api("GET", "/directory"))
);
...
server.tool(
  ...
  {},
  async () => text(await api("GET", "/token"))
);
